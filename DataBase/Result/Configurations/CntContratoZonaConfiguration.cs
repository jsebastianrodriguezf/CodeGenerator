...
public class CntContratoZonaConfiguration : IEntityTypeConfiguration<CntContratoZona>
{
    public void Configure(EntityTypeBuilder<CntContratoZona> builder)
    {
        builder.ToTable("cnt_contrato_zona");

        builder.HasIndex(e => e.Eid, "IX_cnt_contrato_zona");
        ...
        builder.HasOne(d => d.IdContratoNavigation).WithMany(p => p.CntContratoZonas)
            .HasForeignKey(d => d.IdContrato)
            .OnDelete(DeleteBehavior.ClientSetNull)
            .HasConstraintName("FK_cnt_contrato_zona_id_contrato");

        builder.HasOne(d => d.IdZonaNavigation).WithMany(p => p.CntContratoZonas)
